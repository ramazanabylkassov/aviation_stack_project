...
def raw_to_datamart():
    ...
    combined_data = pd.concat([client.query(f"SELECT *, FORMAT_DATE('%A', flight_date) AS weekday, EXTRACT(HOUR FROM your_timestamp_column) AS hour FROM `{source_dataset_id}.{table_id}`").to_dataframe() for table_id in source_table_ids])
